...
**반오토(BahnAuto)** 서비스 홈페이지(`bahnauto.kr`) 운영 정보입니다. 운영사는 (주)우리끼리입니다.
...
| 페이지 검사 | `tools/check-pages.py` | 파일 인코딩 · 파일명 · 페이지 기본 요건 · 내부 링크 · 필수 파일 |

**페이지 검사**는 개발 경험이 없는 팀원이 HTML을 직접 올리는 것을 전제로 만들었습니다.
문제를 찾으면 무엇이 잘못됐고 어떻게 고치는지 한국어로 함께 출력합니다.

| 검사 | 잡는 것 |
|---|---|
| 파일 인코딩 | EUC-KR(ANSI)로 저장된 파일 → 사이트에서 한글 깨짐 |
| 파일 이름 | 한글·공백·대문자 → 주소가 깨짐 |
| 기본 요건 | `<!DOCTYPE>`, `lang`, `<meta charset>`, `<title>` 누락 |
| 내부 링크 | 없는 페이지·이미지로 걸린 링크 |
| 필수 파일 | `CNAME`, `.nojekyll`, `404.html` 삭제 및 `CNAME` 내용 변조 |

네트워크를 쓰지 않아 외부 사이트 사정으로 CI가 깨지지 않고,
