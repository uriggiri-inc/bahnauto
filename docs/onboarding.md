...
**반오토(BahnAuto)** 서비스 홈페이지(`bahnauto.kr`) 개발에 처음 참여할 때 순서대로 따라하면 됩니다.
30분이면 첫 PR까지 갑니다.

> 이 사이트는 **반오토라는 서비스의 공식 홈페이지**입니다. 운영사인 (주)우리끼리의 회사 홈페이지가 아닙니다.
> 화면에 보이는 브랜드는 **반오토**이고, 운영사 표기는 푸터에만 들어갑니다.
...
| ❌ | 브랜드를 '(주)우리끼리'로 표기 — 화면에 보이는 이름은 **반오토**입니다 |
