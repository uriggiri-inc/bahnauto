 ...
 *
 * `/company` 는 **2026-09-07 페이지 자체를 지웠다.** 푸터·홈 버튼이 모두 운영사
 * 홈페이지(uriggiri.kr)로 나가면서(2026-08-18) 들어갈 입구가 사라졌고, 링크 없는
 * 페이지를 사이트맵으로만 밀어 넣으면 품질 신호가 나빠지기 때문이다. 라우트가
 * 없으므로 여기서도 "제외" 가 아니라 그냥 **없는 주소**다.
 *
...
const STATIC_PATHS = [
  ...
  "/app",
  /*
    `/terms` · `/privacy` 는 **2026-09-02 사이트에서 없앴다**(사용자 지시).
    법정 문서의 정본이 노션 원본으로 옮겨졌고(`content/legal/urls.ts`),
    사이트 안의 사본은 원본이 고쳐질 때마다 갈라지기만 했다.

    푸터가 노션으로 바로 나가므로 여기 남겨 두면 **사라진 주소를 색인해 달라고
    요청하는 셈**이 된다. 노션 주소는 우리 사이트맵에 넣지 않는다 — 다른 도메인이라
    이 사이트맵의 관할이 아니다(노션이 자체적으로 처리한다).
  */
];
