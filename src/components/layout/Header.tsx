 ...
 *
 * `서비스`·`운영 시스템`·`도입 사례` 는 GNB 에서 내렸다. 기획의 3개 메뉴에
 * 흡수됐다. 라우트 자체는 남아 있어 기존 링크는 계속 열린다.
 */
 ...
 *
 * ── 왜 문자열 비교로 끝나지 않는가 ──
 * `주요기능` 의 주소는 `/features/dashboard` 인데 방문자는 `/features/place` 에
 * 있을 수 있다. 정확히 같은지만 보면 8기능 중 1번에서만 강조가 켜진다.
  ...
  return (
    ...
    >
      ...
      */}
      ...
      >
        {/*
          로고. 넓은 화면에서는 **테두리만 두른 알약**이다(사용자 지시 2026-08-18) —
          배경·그림자·블러를 뺐다. 메뉴 패널은 글자가 여러 개라 배경이 필요하지만
          로고는 그래픽 하나라 윤곽선만으로 자리가 잡힌다.

          테두리를 `border-border-strong` 으로 올린 이유: 배경이 사라지면서 형태를
          잡아 주는 것이 선 하나뿐이다. 기존 `border-border/60` 은 흰 배경에서
          거의 보이지 않아 "테두리만"이 곧 "아무것도 없음"이 된다.

          좌우 여백은 로고 높이의 0.25배 이상(클리어스페이스 규정)을 지킨다.

          ⚠️ 배경을 뺀 대가: 어두운 섹션(`bg-ink`·`bg-brand`)이 띠 뒤로 지나갈 때
             파란 워드마크와 연한 테두리가 그 색에 묻힌다. 헤더가 `fixed` 라
             본문이 뒤로 흐르기 때문이다. 눈에 걸리면 아주 연한 배경을 되살리는
             것이 가장 싼 해결이다(그때는 "테두리만" 이 아니게 된다).
        ...
        */}
        <Link
          ...
          className={cn(
            ...
            "lg:mr-auto",
            "lg:border-border-strong lg:pointer-events-auto lg:rounded-full lg:border lg:px-5 lg:py-2",
