 ...
 *
 * ⚠️⚠️ **이메일은 개인정보처리방침 제2조의 수집 항목에 아직 없다(X-02).**
 * 2026-09-08 사용자 지시로 접수를 열었다 — 무료체험 폼이 2026-09-04 부터 이미 이메일을
 * 받아 저장하고 있어 소개서만 막아 두는 것이 뜻이 없어졌기 때문이다.
 * 방침 개정 때 **네 폼(무료체험 · 소개서 · 상담 · 지원)의 수집 항목을 함께** 손본다:
 * 이메일 · 연락 가능 시간대 · 유입 경로. 진행 상황은 `docs/영업관리-접수-API.md` 에 있다.
 *
...
/*
  ...
  ── 2026-09-08 `연락 가능 시간대` 는 **일부러 넣지 않았다** (사용자 확정) ──
  그날 무료체험 폼에는 `연락 가능 시간대`(연락처 바로 밑, 필수)가 들어갔다. 소개서에는
  넣지 않는다 — 사용자가 "소개서에는 연락 가능 시간대 추가 안 해도 된다, 무료체험에만
  넣자" 로 정정했다. **빠뜨린 것이 아니다.** 소개서는 사람이 전화를 거는 폼이 아니라
  파일을 보내는 폼이라 통화 가능 시간을 받을 이유가 없다.

  ⚠️ 그래서 **"두 폼을 항상 같게 유지" 는 더 이상 규칙이 아니다.** 함께 맞추는 범위는
     이 다섯 칸(성함 · 연락처 · 이메일 · 회사명 또는 매장명 · 유입 경로)의 **순서와
     라벨**뿐이다. 무료체험 쪽에만 있는 칸을 여기로 옮기지 않는다.
*/
