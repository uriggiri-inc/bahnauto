...
홈페이지의 신청 폼 넷(무료체험·소개서·매니저 지원·도입 상담)은 모두 반오토 영업관리 접수 API 로 간다.
항목을 고치기 전에 [`docs/영업관리-접수-API.md`](docs/%EC%98%81%EC%97%85%EA%B4%80%EB%A6%AC-%EC%A0%91%EC%88%98-API.md) 를 읽는다 —
정의에 없는 키는 저장되지 않고, 보내는 경로가 둘(정적·서버)이라 함께 고쳐야 한다.
