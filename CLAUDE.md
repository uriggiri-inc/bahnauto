...
### 8. 법정 문서는 이 저장소에 없다 (2026-09-02)

이용약관·개인정보처리방침은 **노션 원본이 정본**이고 푸터가 그리로 바로 나간다.
주소는 `content/legal/urls.ts` 한 곳이다.

그 전까지는 사이트 안에 사본(`content/legal/{terms,privacy}.ts`)을 두고
`marketing/LegalDocument.tsx` 로 렌더했다. 법무가 문구를 고칠 때마다 옮겨 적어야
했고 그 사이 두 문서가 어긋났다. **사용자 지시로 사본과 `/terms`·`/privacy`
라우트를 지웠고 사이트맵에서도 뺐다.** 옛 내용이 필요하면 git 이력에 있다.

⚠️ **다시 만들지 않는다.** 사본이 생기는 순간 노션과 갈라지고, 어느 쪽이 진본인지
알 수 없게 되는 것이 개별 문구의 오류보다 위험하다.

⚠️ 두 주소는 **로그인 없이 열려야 한다**(개인정보보호법 제30조 ② 공개 의무).
노션에서 공유를 내리면 그 순간 사이트가 법정 고지를 잃는다.

회사 정보(`content/company.ts`)는 그대로 남아 있다. 실제 법무 문서에서 옮긴
**확정본**이므로 값을 고칠 때는 약관·개인정보처리방침 원본과 반드시 함께 고친다.
